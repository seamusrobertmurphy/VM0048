...
pacman::p_load(
  cols4all, distill, dplyr, flextable, knitr, latex2exp, leaflet, leaflet.extras, 
  leaflet.providers, leafgl, leafem, latticeExtra, maptiles, markdowntemplates, 
  palmerpenguins, PROJ, raster, Rcpp, RcppThread, reproj, rmarkdown, rmapshaper, 
  sf, sp, terra, tidyterra, tinytex, tmap, tmaptools, xaringan, xaringanExtra)
...
-   A project coordinate system was declared as the EPSG:32629[^3] projected reference system, encouraging best practices, while matching package requirements and avoiding delays in shapefile processing downstream.
...
```{r, echo=T, message=F, warning=F, error=F, comment=NA, eval=F}
...
-   All output geometries were checked and repaired systematically using the folloiwng three geometry corrections:
    -   `st_cast()`, `st_zm()`, `st_make_valid()` <!--# Note to self, need to summarize explicitly the geometry effects of each function and the limitations of systematically applying  to all shapefiles, for example zoom in and review the final leakage belt shapefile below that includes so many triangles. Alternatively, we may recommend use of ST_MakeValid(), ensuring OGC Simple Feature compliance -->

```{r, message=F, warning=F, error=F, comment=NA, eval=F, echo=F}
...
1.  Road Data Processing:
    ...
    -   The Douglas-Peucker algorithm[^7] was employed on larger datasets to reduce computational load by simplifying vertex networks, while maintaining structural and linework integrity.
2.  Buffering roads:
...
[^7]: Douglas, D. H., & Peucker, T. K. (1973). Algorithms for the reduction of the number of points required to represent a digitized line or its caricature. *Cartographica: the international journal for geographic information and geovisualization*, *10*(2), 112-122.
